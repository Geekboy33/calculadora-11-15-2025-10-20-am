...
class BalanceStore {
  ...
  updateBalancesRealTime(balances: CurrencyBalance[], fileName: string, fileSize: number, progress: number): void {
    try {
      const orderedBalances = this.orderBalances(balances);
      ...
      const storeData: BalanceStoreData = {
        ...
        fileName: fileName,
        fileSize: fileSize,
        totalTransactions: balances.reduce((sum, b) => sum + b.transactionCount, 0),
      };

      localStorage.setItem(STORAGE_KEY, JSON.stringify(storeData));
      
      // Notify all listeners immediately (Account Ledger, Black Screen, etc.)
      this.notifyListeners(orderedBalances);
      
      if (progress % 10 === 0 || progress === 100) {
        console.log(`[BalanceStore] Real-time update: ${progress.toFixed(1)}% - ${orderedBalances.length} currencies`);
      }
    } catch (error) {
      console.error('[BalanceStore] Error updating balances:', error);
    }
  }
