...
import { exchangeRatesManager } from '../lib/exchange-rates';
...
export function AccountDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeBalances = balanceStore.subscribe((balances) => {
      ...
      // Update ledger accounts when balances change
      if (balances.length > 0) {
        syncLedgerWithBalances(balances);
      }
    });
    ...
    const unsubscribeLedger = ledgerAccountsStore.subscribe((accounts) => {
      ...
      console.log('[AccountDashboard] Ledger accounts updated:', accounts.length);
    });
  ...
  const loadLedgerAccounts = async () => {
    ...
    try {
      ...
      setLedgerAccounts(accounts);
      console.log('[AccountDashboard] Loaded ledger accounts:', accounts.length);
    } catch (error) {
      console.error('[AccountDashboard] Error loading ledger:', error);
    } finally {
  ...
  const syncLedgerWithBalances = async (balances: CurrencyBalance[]) => {
    try {
      ...
      console.log('[AccountDashboard] Ledger synced with balances');
    } catch (error) {
      console.error('[AccountDashboard] Error syncing ledger:', error);
    }
  };
  ...
  const getTotalLedgerBalance = () => {
    return ledgerAccounts.reduce((sum, acc) => {
      const converted = exchangeRatesManager.convert(acc.balance, acc.currency, selectedCurrency);
      return sum + converted;
    }, 0);
  };

  const getConvertedBalance = (amount: number, from: string) => {
    return exchangeRatesManager.convert(amount, from, selectedCurrency);
  };
          ...
          <div className="bg-gradient-to-r from-[#0d0d0d] to-[#0a0a0a] border border-[#00ff88]/30 rounded-xl p-6 glass-panel shadow-[0_0_20px_rgba(0,255,136,0.2)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 max-h-64 overflow-y-auto">
              {ledgerAccounts.map((account, index) => {
                ...
                const convertedBalance = getConvertedBalance(account.balance, account.currency);
                const rate = exchangeRatesManager.getRate(account.currency, selectedCurrency);
